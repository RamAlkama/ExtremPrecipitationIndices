...
function drySpells(img, list){
  ...
  var dry = img.select('total_precipitation').lt(precipThresh);
  // find areas  precipitation threshold (>1)
  var wet = img.select('total_precipitation').gt(precipThresh);
  ...
  var out = img.select('total_precipitation').addBands(img.select('counter').where(dry.eq(1),accum)).uint8()
...
function qualityMask10(img) {
  ...
  var mask = img.select('total_precipitation').gt(0.01);//(10);
  // Update the image mask and return it.
  return img.updateMask(mask);
}

function qualityMask20(img) { var mask = img.select('total_precipitation').gt(0.02);//(20);
                              return img.updateMask(mask);
                             }
                             
function qualityMask1(img) { var mask = img.select('total_precipitation').gt(0.001);//(1);
                              return img.updateMask(mask);
                             }
...
function countPercentile95(img) {var mask = img.select('total_precipitation').gt(precipitation_p95);
                               var prec = img.select('total_precipitation')
                               return prec.updateMask(mask);
                               }

function countPercentile99(img) {var mask = img.select('total_precipitation').gt(precipitation_p99);
                               var prec = img.select('total_precipitation')
                               return prec.updateMask(mask);
                               }  
// ----------------
...
//var collection=collection.select(['total_precipitation'],['precipitation']);//ee.ImageCollection("UCSB-CHG/CHIRPS/DAILY");
var metre_to_mm = function(image) {
  return image.multiply(1000);
};

//var collection = collection.map(metre_to_mm);
//print (collection.first())
...
var p95p99=collection.select("total_precipitation").filterDate('1981-01-01', '2021-01-01').map(qualityMask1).reduce(ee.Reducer.percentile([95,99]));
print (p95p99)
var precipitation_p95=p95p99.select('total_precipitation_p95');
var precipitation_p99=p95p99.select('total_precipitation_p99');
...
var datain_t = collection.filterDate(startdate, enddate)
  ...
  .select("total_precipitation").map(function(img){
...
var scale_p = ee.Image(datain_t.first().select('total_precipitation')).projection().nominalScale()
...
var col=collection.select("total_precipitation").filterDate(startdate, enddate);
var Pmax=col.max()
...
// calculating the max image among those 5 days moving sum
...
Export.image.toDrive({
  ...
  image: Indices.float(),
description: 'ERA5_GloPRCPindex_'+yr, 
folder: 'ERA5',
  crs: 'EPSG:4326',
  //region: ee.Geometry.Polygon([-9, 38, 0, 38, 12, 38, 12, 19, 0, 19, -9, 19], null, false),
  region: ee.Geometry.Polygon([-180, 90, 0, 90, 180, 90, 180, -90, 0, -90, -180, -90], null, false), //Global
  maxPixels: 1e13,
  scale:'27829.8726983'//'5565.974539663679'
  }); 
}
// display results
